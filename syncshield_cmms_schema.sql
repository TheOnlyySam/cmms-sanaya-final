...
ALTER TABLE users ADD COLUMN IF NOT EXISTS company_id uuid REFERENCES company_settings(id) ON DELETE SET NULL;
ALTER TABLE company_settings ADD COLUMN IF NOT EXISTS company_subdomain varchar(63);

CREATE INDEX IF NOT EXISTS idx_users_role_id ON users(role_id);
CREATE INDEX IF NOT EXISTS idx_users_company_id ON users(company_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_company_settings_subdomain ON company_settings(company_subdomain) WHERE company_subdomain IS NOT NULL;
...
INSERT INTO company_settings (company_name, company_subdomain, company_type, report_footer) VALUES
  ('SyncShield', 'syncshield', 'CMMS Operator', 'Confidential maintenance record generated by SyncShield CMMS.');
